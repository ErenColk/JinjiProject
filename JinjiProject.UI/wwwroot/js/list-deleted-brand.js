    ...
    brands.forEach((brand, index) => {
        ...
        brandTR.innerHTML = `
                <td>${brand.id}</td>
                                            ...
                                            <td>${new Date(brand.deletedDate).toLocaleDateString()}</td>
                                            <td>
                                                <button class="btn btn-outline-secondary dropdown-toggle" type="button" id="dropdownMenu1" data-toggle="dropdown">
                                                    Seçenekler
                                                </button>
                                                <ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu">
                                                    <a class="dropdown-item text-success " href="/Admin/Brand/AddAgainBrand/${brand.id}">Tekrar Ekle</a>
                                                    <div class="dropdown-divider"></div>
                                                    <a class="dropdown-item text-danger" onclick="loadBrandData(${brand.id})" id="modalOpener" data-bs-toggle="modal" data-bs-target="#exampleModal" data-id="${brand.id}">Veriyi Sil</a>
                                                </ul>
                                            </td>`;
