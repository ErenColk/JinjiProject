    ...
    subscribers.forEach((subscriber, index) => {
        ...
        subscriberTR.innerHTML = `
                <td>${subscribersPerPage * (currentPage - 1) + index + 1}</td>
                                            ...
                                            <td>${new Date(subscriber.deletedDate).toLocaleDateString()}</td>
                                            <td>
                                                <button class="btn btn-outline-secondary dropdown-toggle" type="button" id="dropdownMenu1" data-toggle="dropdown">
                                                    Seçenekler
                                                </button>
                                                <ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu">
                                                    <a class="dropdown-item text-success " href="/Admin/Subscriber/AddAgainSubscriber/${subscriber.id}">Tekrar Ekle</a>
                                                    <a class="dropdown-item text-danger" onclick="loadSubscriberHardDeleteData(${subscriber.id})" id="modalOpener" data-bs-toggle="modal" data-bs-target="#exampleModal" data-id="${subscriber.id}">Veriyi Sil</a>
                                                </ul>
                                            </td>`;
